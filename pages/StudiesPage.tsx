...
const StudiesPage: React.FC = () => {
  ...
  const getToolButtonClass = (toolId: ActiveStudyTool) => {
    const base = "px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ease-in-out shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-800";
    if (activeTool === toolId) {
      return `${base} bg-purple-600 text-white scale-105 shadow-lg focus-visible:ring-purple-500`;
    }
    return `${base} bg-slate-200 dark:bg-gray-700 text-slate-700 dark:text-gray-300 hover:bg-purple-500 dark:hover:bg-purple-600 hover:text-white hover:shadow-md focus-visible:ring-purple-400`;
  };
  ...
  return (
    <div className="h-full flex flex-col p-2 md:p-4">
      <div className="mb-4 p-3 rounded-lg bg-slate-100 dark:bg-gray-800/60 shadow-md">
      ...
      <div className="flex-grow overflow-hidden rounded-lg bg-white dark:bg-gray-800/30 shadow-inner">
        {selectedToolDetails ? (
          <ChatInterface 
            key={selectedToolDetails.id} // Force re-mount on tool change for clean state
            initialGreetingMessage={selectedToolDetails.greeting}
            chatContext={selectedToolDetails.context}
          />
        ) : (
          <div className="h-full flex flex-col items-center justify-center text-center p-8">
            <h2 className="text-2xl md:text-3xl font-semibold mb-3 text-purple-600 dark:text-purple-400">Suite de Estudios de {AI_NAME}</h2>
            <p className="mb-6 text-slate-600 dark:text-gray-300 max-w-md">
            ...
            <div className="animate-pulse text-purple-500 dark:text-purple-300">
